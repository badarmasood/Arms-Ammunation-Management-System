'''
Semester Project
Names: Badar Masood(Sp19-bcs-051)
Title: Arms and Ammunation Management System
'''
# Entity1:Arm
# Entity2:Buyer
...
def add_record():
    ...
    while confirmation == "Y" or confirmation == "y":
        ...
        total_arms = int(
            input("Enter the amount records you want to create about arms:"))
        a = 1
        print()
        for i in range(total_arms):
            ...
            arm_price = eval(
                input("Enter arm price of arm_" + str(a)+" here:"))
        ...
        for j in range(total_owner):
            ...
            contact_no = eval(
                input("Enter buyer's contact number of buyer_"+str(b)+" here:"))
            owner_list.append(contact_no)
            id_no = eval(
                input("Enter buyer's id card number of buyer_"+str(b)+" here:"))
            owner_list.append(id_no)
            arm_model = input(
                "Enter model of arm parchased by buyer_"+str(b)+" here:")
            owner_list.append(arm_model)
            b += 1
            print()
        confirmation = input(
            "Enter \"Y\" to continue to add more records and press any other key to move :")
    print(arm_list)
    print(owner_list)
...
def view_record():
    print("---------------VIEW RECORD ------------------------")
    choice = eval(
        input("Enter \"1\" to view record of arms and \"2\" to view record of owners:"))
    if choice == 1:
        print()
        print("arm_name\t\tarm_price\t\tarm_model\t\tarm_color")
        for i in range(0, len(arm_list), 4):
            print(arm_list[i], "\t\t", arm_list[i+1], "\t\t",
                  arm_list[i+2], "\t\t", arm_list[i+3])
    if choice == 2:
        print("buyer_name\t\tcontact_no\t\tid_no\t\tarm_model")
        for i in range(0, len(arm_list), 4):
            print(owner_list[i], "\t\t", owner_list[i+1],
                  "\t\t", owner_list[i+2], "\t\t", owner_list[i+3])


def edit_record():
    choice1 = eval(input(
        "Enter \"1\" to edit the record of arms and \"2\" to edit the record of owners:"))
    if choice1 == 1:
...
def search_record():
    print("---------------SEARCH RECORD-----------------------")
    choice2 = eval(input(
        "Enter \"1\" to search the record of arms and \"2\" to search the record of owners:"))
    if choice2 == 1:
...
def user_choice():
    ...
    while choice_confirm == "Y" or choice_confirm == "y":
        ...
        choice_confirm = input(
            "Enter \"Y\" to continue the choice of User or press any other key to End The Program:")
